...
* If the pod wont ready up and you see the pod's status as 'ErrImagePull', then:
    - do a describe and check the event logs: e.g. `k describe pod ref-net-core-api-855b755cc8-rdm9h`
    - This can show a 401 issue with getting the image:
  ```Events:
...
url should be:
http://ingress.local.kavm.com.au/api/ref-app/blogs

If you get nothing back:

- check if the api is working locally:
get a shell into one of the pods and poke around: e.g. `kubectl exec -i -t -n kavm-services ref-net-core-api-855b755cc8-xfs5h -c ref-net-core-api -- sh -c "clear; (bash || ash || sh)" ` 
...
do check the logs of the pod to see if there's a database error if you're not getting anything back:
e.g.
